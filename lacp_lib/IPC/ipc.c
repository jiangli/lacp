...
#define IPC_SET_SOCKET_CLIENT_ID(SPTR, X)       \
...
int
IPC_SocketInit (IPC_SOCKET_T * s,
		IPC_SOCK_ID socket_id, TYPE_OF_BINDING binding)
{
  bzero (s, sizeof (IPC_SOCKET_T));
...
int
IPC_SocketRecvfrom (IPC_SOCKET_T * sock,
		    void *msg, int buffer_size, IPC_SOCKET_T * sock_4_reply)
{
  int size;
  ...
  while (1) {
    size = recvfrom (sock->sock_fd, msg, buffer_size, 0,
		     ((IPC_BIND_AS_CLIENT == sock->binding)
		      || !sock_4_reply) ? NULL : &(sock_4_reply->clientAddr),
		     (IPC_BIND_AS_CLIENT == (sock->binding)) ? NULL : &len);
    if (size < 0 && errno == EINTR)
      continue;
    break;
  }

  if ((IPC_BIND_AS_CLIENT != sock->binding) && sock_4_reply) {
    sock_4_reply->sock_fd = sock->sock_fd;
    sock_4_reply->binding = IPC_BIND_AS_SERVER;
  }
...
int
IPC_SocketSendto (IPC_SOCKET_T * sock, void *msg, int msg_size)
{
  int rc, size = 0;
  ...
  while (size != msg_size) {
    rc = sendto (sock->sock_fd, (msg + size), (msg_size - size), 0,
		 (IPC_BIND_AS_CLIENT ==
		  sock->binding) ? &sock->serverAddr : &sock->clientAddr,
		 SIZE_OF_ADDRESS);
...
int
IPC_SocketClose (IPC_SOCKET_T * s)
{
  close (s->sock_fd);
...
int
IPC_SocketSetReadTimeout (IPC_SOCKET_T * s, int timeout)
{
  int retval = -1;
  struct timeval wait;
  fd_set read_mask;
  ...
  if (retval < 0) {		// Error
    fprintf (stderr, "IPC_SocketSetReadTimeout failed: %s\n",
	     strerror (errno));
    return 0;
